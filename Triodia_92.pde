...
// press Y to count the Yellow Pixels
...
void setup() {
  ...
  img.resize(0, 1500);  // larger number makes for less reduction use 3000 for 1:1 scale or just comment out this line
...
void keyPressed() {
  ...
  if (key == 'n' || key == 'N') {
    ...
    ellipse(pX[currentPerimeterPoint], pY[currentPerimeterPoint], 2, 2);  // small ellipse at start point
  ...
  if (key == 'o' || key == 'O') {
    ...
    textSize(20);
    fill(255, 255, 0); // Text Colour yellow
  ...
  if (key == 'y' || key == 'Y') {
    countYelowPixels();
    countOfYelowPixels = initialCountOfYellowPixels;
...
int countYelowPixels() {
  ...
  println("Area = " + area);
  // print Area on the screen at printAreaAtX,printAreaAtY
  area =  float(nf((countOfYelowPixels / pixelsPerSqMeter), 0, 2));
  String areaST = str(area);
  fill(0);
  stroke(0);

  if (closedPerimiter == true) {
    text(areaST, mouseX, mouseY);
  }
